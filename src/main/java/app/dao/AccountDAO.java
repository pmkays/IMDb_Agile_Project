...
public class AccountDAO {
			...
			while (result.next()) {
				...
				accounts.add(
						...
						new Account(result.getString("username"), result.getString("password"),
								...
								result.getString("role"), result.getString("post_code"), result.getString("year"),
								result.getString("organisation_name"), result.getString("organisation_number"),
								result.getInt("status"), result.getString("proco_id"))
		...
		try {
			...
			if (acct.getProcoId() == null) {
				sql = String.format(
						...
						acct.getUsername(), acct.getPassword(), acct.getEmail(), acct.getCountry(), acct.getGender(),
						acct.getFirstName(), acct.getLastName(), acct.getRole(), acct.getPostCode(),
						acct.getOrganisationName(), acct.getOrganisationNumber(), acct.getYear(), acct.getStatus());
			} else {
				sql = String.format("INSERT INTO `account` (`username`, `password`, "
						...
						+ "('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',%d, %s);", acct.getUsername(),
						acct.getPassword(), acct.getEmail(), acct.getCountry(), acct.getGender(), acct.getFirstName(),
						acct.getLastName(), acct.getRole(), acct.getPostCode(), acct.getOrganisationName(),
	...
	public static boolean updateAccountStatus(String username, String status) {
		boolean toReturn = false;
		Account check = getUserByUsername(username);
		if (check == null || username == null || status == null) {
			return false;
		}
